...
void RIT_IRQHandler (void) {					
	...
	if (WITHIN_INTERVAL(lcd.x, LCD_START_START_X, LCD_START_END_X) && WITHIN_INTERVAL(lcd.y, LCD_START_START_Y, LCD_START_END_Y)) {
		...
		LCD_DrawRect(60, 110, 50, 120, White); // Clears words on the map area
		...
		if (is_exit_reached(game)) {
			...
			GUI_Text(20, 80,  (uint8_t *)"!!!!!!!!!!!!!!!!!!!!!!!!!", Red, White);
			GUI_Text(20, 155, (uint8_t *)"!!!!!!!!!!!!!!!!!!!!!!!!!", Red, White);
			GUI_Text(100, 110, (uint8_t *)"YOU", Red, White);
			GUI_Text(95, 125, (uint8_t *)"WON!!", Red, White);
		}
			...
			for (i = 1; i < NROWS - 2; i++) {
				for (j = 1; j < NCOLS - 2; j++) {
					if (game.map[i][j] == OBSTACLE && !(j == game.robot.obstacle_pos.col && i == game.robot.obstacle_pos.row)) {
						LCD_DrawRect(LCD_POSCOL(j-1), LCD_POSROW(i-1), 16, 16, White);
					}
				}
			}				
		}
